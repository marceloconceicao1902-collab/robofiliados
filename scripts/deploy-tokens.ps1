...
# REQUISITO: Antes de rodar, defina AS 3 VARS DE AMBIENTE ABAIXO no terminal que for rodar:
...
param(
  ...
  [string]$FirebaseProjId   = "robofiliados-$([DateTime]::Now.ToString('yyMMddHHmm'))",
...
# ---------- 3. Firebase Functions (create project + build functions + deploy) ----------
...
# ---------- FINAL: RESUMO ----------
Write-Step "========================================="
Write-Host "  🎉  DEPLOY CONCLUIDO  " -ForegroundColor Magenta
Write-Step "========================================="
Write-Host "  🐙  GitHub   : https://github.com/$GithubOwner/$GithubRepo"
Write-Host "  ▲   Vercel   : $vercelUrl  (testa GET /api/health)"
